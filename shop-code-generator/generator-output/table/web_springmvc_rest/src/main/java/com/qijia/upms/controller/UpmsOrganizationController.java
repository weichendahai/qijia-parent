 ...
 * 标准的rest方法列表
 * <pre>
 * /upmsorganization                => index()  
 * /upmsorganization/new            => _new()  注意: 不使用/userinfo/add => add()的原因是ad会被一些浏览器当做广告URL拦截
 * /upmsorganization/{id}           => show()  
 * /upmsorganization/{id}/edit      => edit()  
 * /upmsorganization        POST    => create()  
 * /upmsorganization/{id}   PUT     => update()  
 * /upmsorganization/{id}   DELETE  => delete()  
 * /upmsorganization        DELETE  => batchDelete()  
 * </pre>
 *
 * @author weichen
 * @version 1.0
...
public class UpmsOrganizationController {
	...
	private final String LIST_ACTION = "redirect:/upmsorganization";
	...
	/** 列表 */
	@RequestMapping
	public String index(ModelMap model,UpmsOrganizationQuery query,HttpServletRequest request) {
	...
	public String show(ModelMap model,@PathVariable Integer id) throws Exception {
		...
		return "/upmsorganization/show";
	}

	/** 进入新增 */
	@RequestMapping(value="/new")
	public String _new(ModelMap model,UpmsOrganization upmsOrganization) throws Exception {
		model.addAttribute("upmsOrganization",upmsOrganization);
		return "/upmsorganization/new";
	}
	
	/** 保存新增,@Valid标注spirng在绑定对象时自动为我们验证对象属性并存放errors在BindingResult  */
	@RequestMapping(method=RequestMethod.POST)
	public String create(ModelMap model,UpmsOrganization upmsOrganization,BindingResult errors) throws Exception {
		try {
	...
	public String edit(ModelMap model,@PathVariable Integer id) throws Exception {
		UpmsOrganization upmsOrganization = (UpmsOrganization)upmsOrganizationManager.getById(id);
		model.addAttribute("upmsOrganization",upmsOrganization);
		return "/upmsorganization/edit";
	}
	
	/** 保存更新,@Valid标注spirng在绑定对象时自动为我们验证对象属性并存放errors在BindingResult  */
	@RequestMapping(value="/{id}",method=RequestMethod.PUT)
	public String update(ModelMap model,@PathVariable Integer id,UpmsOrganization upmsOrganization,BindingResult errors) throws Exception {
		try {
	...
	public String batchDelete(ModelMap model,@RequestParam("items") Integer[] items) {
		for(int i = 0; i < items.length; i++) {
			upmsOrganizationManager.removeById(items[i]);
		}
		Flash.current().success(DELETE_SUCCESS);
		return LIST_ACTION;
	}
