...
<script>
    import { fetchList, deleteUpmsUserRole, insertUpmsUserRole, updateUpmsUserRole} from 'api/upmsuserrole';

    export default {
        name: 'UpmsUserRole_INDEX',
        data() {
            return {
                list: null,
                total: null,
        ...
        methods: {
            ...
            handleCreate () {
                insertUpmsUserRole (this.temp).then(response => {
            ...
            // update() {
            handleUpdate () {
                updateUpmsUserRole (this.temp).then(response => {
                    this.dialogFormVisible = false;
                    this.$notify({
            ...
            handleDelete(row) {
                this.$confirm('确认删除用户 [ ' + row.id +' ]？')
                    .then(_ => {
                        ...
                        deleteUpmsUserRole (postData).then(response => {
                            this.$notify({
